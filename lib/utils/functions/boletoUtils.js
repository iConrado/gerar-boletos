...
var Datas = (function () {
	...
	Datas.prototype.comDocumento = function (_documento,locate = 'usa') {
		if (locate === 'usa') {
			_documento = new Date (moment(_documento, 'YYYY-MM-DD'));
		}else if(locate === 'brl'){
	...
	Datas.prototype.comProcessamento = function (_processamento, locate = 'usa') {
		if (locate === 'usa') {
			_processamento = new Date (moment(_processamento, 'YYYY-MM-DD'));
		}else if(locate === 'brl'){
	...
	Datas.prototype.comVencimento = function (_vencimento, locate = 'usa') {
		if(locate == 'usa'){
			_vencimento = new Date (moment(_vencimento, 'YYYY-MM-DD'));
		}else if(locate == 'brl'){
...
var Boleto = (function () {
	...
	Boleto.prototype.getMensagens = function () {
		return this._mensagens || [];
	};
	
	Boleto.prototype.comMensagens = function (_mensagens) {
		if (arguments.length > 1) {
			_mensagens = Array.prototype.slice.call(arguments, 0);
		}

		if (typeof _mensagens === 'string') {
			_mensagens = [_mensagens];
		}

		if (_mensagens.length > 10) {
			throw new Error('Máximo de dez mensagens permitidas');
		}
	...
	Boleto.prototype.comValorDeducoes = function (_valorDeducoes) {
		console.log('_valorDeducoes', _valorDeducoes);
		console.log('_valorDeducoes', isNaN(_valorDeducoes));
		if (!isNaN(_valorDeducoes)) this._valorDeducoes = _valorDeducoes;
		return this;
	};
